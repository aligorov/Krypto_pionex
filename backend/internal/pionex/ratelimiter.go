...
// Wait blocks until the required weight tokens are available.
func (rl *RateLimiter) Wait(ctx context.Context, weight int) error {
	...
	if now.Before(rl.cooldownUntil) {
		return fmt.Errorf("rate limiter in HTTP 429 cooldown until %s", rl.cooldownUntil.Format(time.RFC3339))
	}
	...
	waitDuration := time.Duration(float64(needed)/float64(rl.refillRate)*1000) * time.Millisecond
